...
std::vector<PairConstraintData> buildPairConstraintData(
    ...
    for (int i = 0; i < n; ++i) for (int j = i + 1; j < n; ++j) {
        ...
        for (int fi = 0; fi < 2; ++fi) for (int fj = 0; fj < 2; ++fj) {
            ...
            appendSet(si->finiteTriangles, sj->finiteTriangles, settings.clearance, settings.geomEpsilon, pair.finite[fi][fj], stats);
            pair.finite[fi][fj].finalize(settings.mergeEpsilon);
            pair.hotWire[fi][fj] = pair.finite[fi][fj];
            appendSet(si->extendedTriangles, sj->finiteTriangles, settings.clearance, settings.geomEpsilon, pair.hotWire[fi][fj], stats);
            appendSet(si->finiteTriangles, sj->extendedTriangles, settings.clearance, settings.geomEpsilon, pair.hotWire[fi][fj], stats);
